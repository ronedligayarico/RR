...
<%
	...
	int quantity = 0;
%>
<table class="table table" id="allitemTBL">
		...
		<tr>
			...
			<th>Action</th>
				...
				<tbody>
				<%
					querySelect = "SELECT 	`original_item_id`,`original_item_pic`,`quantity`,`original_item_name`,`branch_name`,`appliances_type_name` FROM `original_item` INNER JOIN `branch` ON `original_item`.`branch_id`=`branch`.`branch_id` INNER JOIN `appliances_type` ON `original_item`.`appliances_type_id`=`appliances_type`.`appliances_type_id` WHERE `original_item`.`markasdeleted`='0'";
				...
				%>
				<tr>	
				<td><img src="../../../RR/components/branch/itemUpload/<%=original_item_pic%>" alt="Rounded Image" class="rounded img-fluid" style="height: 110px; width: 110px;"></td>
